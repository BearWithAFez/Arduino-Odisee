...
void setup() {
    Wire.begin();
    Serial.begin(9600);
    setTime(sec, minu, uur, PM);
    Serial.println(" OEF3 - Dwight vdv");
    Serial.println("-------------------");
}

void loop() {
    byte second, minute, hour, ampm;

    getTime(&second, &minute, &hour, &ampm);
...
void setTime(byte second, byte minute, byte hour, byte ampm) {
    byte temp;
...
void getTime(byte *second, byte *minute, byte *hour, byte *ampm) {
    byte temp;
